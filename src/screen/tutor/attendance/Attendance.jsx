...
import { Clock, Users, CheckCircle, XCircle, Plus, Eye, Copy, CheckCheck, AlertCircle, RefreshCw } from "lucide-react"
...
import { createAttendance, getAttendanceDetails, getClass, closeAttendance, generateCode } from "../../../api/auth"
...
export default function TutorAttendancePage() {
    ...
    const handleCreateAttendance = async () => {
        ...
        try {
            ...
            // Refresh the classes to show the new attendance session
            fetchClasses()

            // Expand the class to show the attendance details
            toggleClassExpansion(selectedClass)
    ...
    const toggleClassExpansion = async (classId) => {
        ...
        // If we're expanding and don't have attendance details yet, fetch them
        if (!expandedClasses[classId] && !attendanceDetails[classId]) {
            await fetchAttendanceDetails(classId)
        }
    }
    ...
    const fetchAttendanceDetails = async (classId) => {
        ...
        try {
            ...
            if (response.data) {
                // Store the attendance details for this class
            ...
            // Don't show an error toast here as the class might not have attendance yet
        } finally {
            setIsLoading(false)
        }
    }
    ...
    const handleGenerateCode = async (sessionId) => {
        setIsLoading(true)
        try {
            const response = await generateCode(sessionId)

            toast({
                title: "Success",
                description: "Attendance codes generated successfully",
            })

            // Update the attendance details for this session
            const classId = classes.find(
                (cls) => attendanceDetails[cls._id] && attendanceDetails[cls._id].sessionId === sessionId,
            )?._id
            ...
            toast({
                title: "Error",
                description: "Failed to generate attendance codes",
                variant: "destructive",
            })
        } finally {
            setIsLoading(false)
    ...
    const handleCloseAttendance = async (sessionId) => {
        ...
        try {
            ...
            // Refresh the classes to update the attendance status
            fetchClasses()
            fetchPastAttendanceSessions()
                        ...
                        <div className="grid gap-4 py-4">
                            <div className="grid gap-2">
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        {classes
                                            .filter((c) => !attendanceDetails[c._id]?.sessionId)
                                            .map((cls) => (
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            {isLoading && classes.length === 0 ? (
                                ...
                                classes.map((cls) => {
                                    const hasAttendance = !!attendanceDetails[cls._id]?.sessionId
                                    const isExpanded = expandedClasses[cls._id]
                                    const attendanceData = attendanceDetails[cls._id]

                                    return (
                                        <div key={cls._id} className="mb-4">
                                            <Card className={`border-l-4 ${hasAttendance ? "border-l-green-500" : "border-l-slate-400"}`}>
                                                    ...
                                                    <div className="mt-4 flex justify-between items-center">
                                                        {!hasAttendance ? (
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                {attendanceData?.signInCodeUsed && (
                                                                    <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                                                                        Sign-In Used
                                                                    </Badge>
                                                                )}
                                                                {attendanceData?.signOutCodeUsed && (
                                                                    <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                                                                        Sign-Out Used
                                                                    </Badge>
                                                                )}
                                                        ...
                                                        <Button
                                                            size="sm"
                                                            variant="ghost"
                                                            onClick={() => toggleClassExpansion(cls._id)}
                                                            className="ml-auto"
                                                        >
                                            ...
                                            {isExpanded && (
                                                <Card className="mt-2 border-t-0 rounded-t-none shadow-sm">
                                                    <CardContent className="p-4">
                                                        {isLoading ? (
                                                            ...
                                                            <div className="text-center py-4">
                                                                <p className="text-muted-foreground mb-4">No attendance session found for this class</p>
                                                                <Button
                                                                    onClick={() => {
                                                                        setSelectedClass(cls._id)
                                                                        setSelectedCourse(cls.course._id)
                                                        ...
                                                        ) : !attendanceData.signInCode ? (
                                                            <div className="text-center py-4">
                                                                <p className="text-muted-foreground mb-4">
                                                                    Attendance session created but no codes generated yet
                                                                </p>
                                                                <Button onClick={() => handleGenerateCode(attendanceData.sessionId)}>
                                                                    <Plus className="mr-2 h-4 w-4" />
                                                                    Generate Codes
                                                                </Button>
                                                            </div>
                                                            ...
                                                            <div className="space-y-4">
                                                                <div className="grid grid-cols-2 gap-4">
                                                                    <div className="bg-secondary/20 p-3 rounded-md">
                                                                        <div className="flex justify-between items-center mb-1">
                                                                            ...
                                                                            <div className="flex items-center gap-1">
                                                                                <Button
                                                                                    ...
                                                                                    size="icon"
                                                                                    className="h-6 w-6"
                                                                                    onClick={() => handleCopyCode(attendanceData.signInCode)}
                                                                                >
                                                                                    {copiedCode === attendanceData.signInCode ? (
                                                                        ...
                                                                        <div className="flex justify-between items-center mt-1">
                                                                            <div className="text-xs text-muted-foreground">
                                                                                {attendanceData.signInCodeUsed ? "Code has been used" : "Code not used yet"}
                                                                            </div>
                                                                            <Button
                                                                                variant="ghost"
                                                                                size="sm"
                                                                                className="h-6 text-xs px-2"
                                                                                onClick={() => handleGenerateCode(attendanceData.sessionId)}
                                                                                disabled={isLoading}
                                                                            >
                                                                    ...
                                                                    <div className="bg-secondary/20 p-3 rounded-md">
                                                                        <div className="flex justify-between items-center mb-1">
                                                                            ...
                                                                            <div className="flex items-center gap-1">
                                                                                <Button
                                                                                    ...
                                                                                    size="icon"
                                                                                    className="h-6 w-6"
                                                                                    onClick={() => handleCopyCode(attendanceData.signOutCode)}
                                                                                >
                                                                                    {copiedCode === attendanceData.signOutCode ? (
                                                                                        <CheckCheck className="h-4 w-4 text-green-500" />
                                                                                    ) : (
                                                                                        <Copy className="h-4 w-4" />
                                                                                    )}
                                                                        ...
                                                                        <div className="text-lg font-mono bg-background p-2 rounded border text-center">
                                                                            {attendanceData.signOutCode}
                                                                        </div>
                                                                        <div className="flex justify-between items-center mt-1">
                                                                            <div className="text-xs text-muted-foreground">
                                                                                {attendanceData.signOutCodeUsed ? "Code has been used" : "Code not used yet"}
                                                                            </div>
                                                                            <Button
                                                                                variant="ghost"
                                                                                size="sm"
                                                                ...
                                                                <div className="mt-4">
                                                                    <div className="flex justify-between items-center mb-1">
                                                                        ...
                                                                        <span className="text-sm font-medium">
                                                                            {attendanceData.signedStudentsCount}/{attendanceData.totalStudentsInClass} Present
                                                                        </span>
                                                                    </div>
                                                                    <Progress
                                                                        value={
                                                                            (attendanceData.signedStudentsCount /
                                                                                Math.max(attendanceData.totalStudentsInClass, 1)) *
                                                                            100
                                                                        }
                                                                        className="h-2"
                                                                    />
                                                                ...
                                                                {attendanceData.attendanceRecords && attendanceData.attendanceRecords.length > 0 ? (
                                                                    <div className="mt-4">
                                                                        <h4 className="text-sm font-medium mb-2">Student Attendance</h4>
                                                                        <Table>
                                                                            <TableHeader>
                                                                                <TableRow>
                                                                ...
                                                                <div className="flex justify-end mt-4">
                                                                    <Button
                                                                        variant="destructive"
                                                                        size="sm"
                                                                        onClick={() => handleCloseAttendance(attendanceData.sessionId)}
                                                                        disabled={isLoading}
                                                                    >
                                                                        Close Attendance
                                                                    </Button>
                                            ...
                                            <TableRow key={session._id}>
                                                ...
                                                <TableCell className="text-right">
                                                    <Button
                                                        size="sm"
                                                        variant="ghost"
                                                        onClick={() => toggleClassExpansion(session.classSession._id)}
                                                        disabled={isLoading}
                                                    >
